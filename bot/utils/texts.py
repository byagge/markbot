...
def format_rating_result(
    ...
    return (
        ...
        f"{line('memo', 'Описание профиля', scores.bio, 15, scores.bio_note)}\n"
...
def anonymity_text(is_on: bool) -> str:
    if is_on:
        status = f"{e('check')} <b>Включена</b>"
        note = (
            "Твой профиль <b>скрыт</b> из всех рейтингов и топов.\n"
            "Оценка себя и других работает как обычно."
        )
    else:
        status = f"{e('cross')} <b>Выключена</b>"
        note = (
            "Твой профиль <b>виден</b> в топах после оценки себя.\n"
            "Включи анонимность, если не хочешь светиться в рейтингах."
        )
    return (
        f"{e('user')} <b>Анонимность</b>\n\n"
        f"Статус: {status}\n\n"
        f"{note}"
    )
...
def rate_other_prompt() -> str:
    return (
        ...
        "Как найти человека:\n"
        f"• Нажми <b>👤 Выбрать пользователя</b> (кнопка снизу)\n"
        "• Или пришли юзернейм / ссылку: <code>@durov</code>, <code>t.me/durov</code>\n"
        "• Или перешли любое его сообщение\n\n"
        f"{e('warning')} <i>Контакт из телефонной книги без Telegram не подойдёт. "
        "Пересылка или кнопка выбора — самый надёжный способ.</i>"
    )
...
def rating_info_text() -> str:
    return (
        ...
        "Количество, модель NFT, редкость и стоимость в Stars\n\n"
        f"{e('memo')} <b>Описание профиля</b> — до 15 баллов\n"
        "Заполненность, ссылки, emoji-статус\n\n"
