...
def rate_other_prompt() -> str:
    return (
        ...
        "Как найти человека:\n"
        f"• Нажми <b>Выбрать пользователя</b> ниже\n"
        "• Или пришли юзернейм / ссылку: <code>@durov</code>, <code>t.me/durov</code>\n"
        "• Или перешли любое его сообщение\n\n"
        f"{e('warning')} <i>Telegram не даёт ботам открыть чужой аккаунт "
        "только по @username. Для подарков и фото используй "
        "<b>Выбрать пользователя</b> или пересылку сообщения.</i>"
    )
